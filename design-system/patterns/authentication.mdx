...
The mukoko ecosystem uses **Stytch B2C** for authentication. The primary method is **Email OTP** — it works everywhere including Zimbabwe where SMS delivery is unreliable. **SMS OTP** is the secondary method for markets with reliable SMS. Session durations: **1 year** for native apps (mobile/desktop), **1 month** for web.
...
Use a single "Full name" field. Do not split into first/last name — African naming conventions are diverse and do not always fit the first/last pattern. See [Inclusive Language](/design-system/content/inclusive-language) for details.
